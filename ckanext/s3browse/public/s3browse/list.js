  ...
  $.get(s3_rest_url)
      .done(function(data) {
        ...
        if (info.nextMarker != "null") {
          ...
          document.getElementById('listing').innerHTML = prepareTable(info);
...
function prepareTable(info) {
  ...
  content.push('    <table class="table table-bordered table-hover table-striped" id="tb-s3objects">');
  content.push('        <thead>');
  content.push('            <tr>');
  content.push('                  <th>Object</th>');
  content.push('                  <th>Last Modified</th>');
  content.push('                  <th>Size</th>');
  content.push('              </tr>');
  content.push('        </thead>');
  content.push('        <tbody id="tbody-s3objects">');
...
function renderRow(item, cols) {
  ...
  // Format the date to local time and display as 'YYYY-MM-DD HH:MM:SS'
  let date = new Date(item.LastModified);
  let localDate = date.toLocaleString("en-CA", {
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
      hour: '2-digit',
  ...
  if (item.keyText.endsWith('/')) {
    row += '    <td class="sorting_2"><a data-s3="object" href="' + item.href + '">' + item.keyText + '</a></td>'
    row += '    <td></td>'
    row += '    <td></td>'
  } else {
    row += '    <td class="sorting_2"><a data-s3="object" target="_blank" href="' + item.href + '">' + item.keyText + '</a></td>'
    row += '    <td>' + localDate + '</td>'
    row += '    <td>' + item.Size + '</td>'
  }
  row += '</tr>'
