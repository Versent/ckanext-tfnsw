...
class OpenApiViewPlugin(p.SingletonPlugin):
    ...
    # constants
    supported_formats = ["openapi-json", "api"]
    ...
    def has_view_already(self, context, resource):
        if "id" in resource:
            ...
            resourceViews = p.toolkit.get_action("resource_view_list")(context, params)
            for resourceView in resourceViews:
                if (
                    "view_type" in resourceView
                    and resourceView["view_type"] == self.view_type
                ):
                    return True
